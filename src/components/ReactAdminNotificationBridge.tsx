...
export const ReactAdminNotificationBridge = () => {
  ...
  useEffect(() => {
    // Override the default notify function to use our Tailwind notifications
  ...
  }, [showSuccess, showError, showWarning, showInfo, notify]);
...
export const useEnhancedNotifications = () => {
  ...
  return {
    ...
    notifyInfo: (message: string) => 
      tailwindNotify(message, { type: 'info' }),
    
    // CRUD operation notifications
    notifyCreated: (resourceName: string, id?: any) => 
      showSuccess('Created', `${resourceName} created successfully${id ? ` (ID: ${id})` : ''}`, { duration: 4000 }),
    
    notifyUpdated: (resourceName: string, id?: any) => 
      showSuccess('Updated', `${resourceName} updated successfully${id ? ` (ID: ${id})` : ''}`, { duration: 4000 }),
    
    notifyDeleted: (resourceName: string, id?: any) => 
      showSuccess('Deleted', `${resourceName} deleted successfully${id ? ` (ID: ${id})` : ''}`, { duration: 4000 }),
    
    notifyValidationError: (message = 'Please check the form for errors') => 
      showError('Validation Error', message),
    
    notifyNetworkError: (message = 'Network error. Please check your connection.') => 
      showError('Network Error', message),
    
    notifyPermissionError: (message = 'You do not have permission to perform this action') => 
